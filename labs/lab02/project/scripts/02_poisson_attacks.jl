# # Параметрическое исследование и дополнительные задания
#
# Зависимость P(N > 10) от интенсивности λ.
# Дополнительно: нестационарный поток, редкие события,
# бутстреп, прореженный поток.
...
# ## Цикл по значениям λ
#
# Для каждого λ: загрузка или симуляция, четыре графика, сводные данные.
...
# ## Дополнительные задания (раздел 2.7)

# ### Задание 2: нестационарный поток λ(t) = 2 + 5·sin(πt/12)
#
# Используем метод прореживания: генерируем однородный поток с верхней
# границей λ_max = 7, каждое событие принимаем с вероятностью λ(t)/λ_max.
...
# ### Задание 3: редкие события
#
# Исследуем вероятности P(0 атак за 8 ч) и P(≥3 атак за 30 мин).
...
# ### Задание 4: доверительный интервал методом бутстрепа
#
# Многократное пересэмплирование исходной выборки с возвращением.
# Из распределения оценок извлекаем квантили 0.025 и 0.975 — 95% ДИ.
...
# ### Задание 5: прореженный поток
#
# Каждая атака имеет вероятность успеха p. Успешные атаки образуют
# пуассоновский поток с интенсивностью λ·p (теорема о прореживании).
